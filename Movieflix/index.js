...
async function searchMovies() {
    ...
    let res = await fetch(`https://www.omdbapi.com/?s=${searchInput.value.trim()}&apikey=f8e33832`)
    let data = await res.json()

    const movies = data.Search

    movies.forEach(async (movie) => {
        let response = await fetch(`https://www.omdbapi.com/?i=${movie.imdbID}&apikey=f8e33832`)
